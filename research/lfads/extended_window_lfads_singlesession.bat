...
set LFADS_SAVE_DIR=C:\Users\hst\Documents\cguan\LFADS_results_extended_window\FingerTapping_singlesession_%SESSION_DATE%_no_inputs_F%FACTORS_DIM%_N%GEN_DIM%_ICD%IC_DIM%_ICED%IC_ENC_DIM%_BS%BATCH_SIZE%_WIS%WEIGHT_INCREASE_STEPS%_KP%KEEP_PROB%

python run_lfads.py --kind=train ^
--data_dir=Y:\Users\Charles\LFADS_data_100ms_to_1500ms ^
...
echo Start time: %START%, End time: %END% > %LFADS_SAVE_DIR%\timing.txt